...
| 2026-03-29 | ドキュメント整理: `sytem-documentions/` を `docs/spec/` + `docs/DEVELOPMENT-PROGRESS.md` に再配置、`docs/research/` 新設、`エージェントルール.md` / `AGENTS.md` 追加 |
...
ドキュメント索引: [docs/README.md](./README.md) · エージェント向け: [エージェントルール.md](../エージェントルール.md)
...
| DEV-00-09 | 受講者ホーム（Skill Hub） | `/learner/skill-hub/home`（旧 `/learner/home` は redirect） | 完了 |
| DEV-00-10 | 受講者コースカタログ | `/learner/track/courses`（旧 `/learner/skills/courses` は redirect） | 完了 |
| DEV-00-11 | コース閲覧（browse） | `/learner/track/courses/browse` | 完了 |
| DEV-00-12 | 受講者 workspace | `/learner/workspace` | 完了 |
| DEV-00-13 | 管理者 ⇄ 受講者ビュー切替（Header） | `src/components/layout/Header.tsx` | 完了 |
| DEV-00-14 | Skill Hub / Track レイアウト分離・コース開始サンプル | `app/learner/skill-hub/`, `app/learner/track/`, `LearnerProgressContext` | 完了 |
...
| ID | タスク | 出力 / 対象 | ステータス |
|----|--------|-------------|------------|
| DEV-D01 | `04-PAGES-AND-FEATURES.md` を実ルートに合わせて更新（Skill Hub / Track 詳細） | `docs/spec/04-PAGES-AND-FEATURES.md` | 未着手 |
| DEV-D02 | `00-REBUILD-SPECIFICATION.md` の app/ ツリー詳細を実態に合わせる | `docs/spec/00-REBUILD-SPECIFICATION.md` | 一部完了（ルート表 2026-03 更新） |
| DEV-D03 | Skill Hub × Track 統合のプロトタイプ設計書を追加（Miro・動画・モック同期・フェーズ） | `docs/spec/06-SKILL-HUB-TRACK-INTEGRATION-PROTOTYPE-DESIGN.md`（新規） | 未着手 |
| DEV-D04 | エージェント用プロンプト・ルールに設計書パスを追記 | `AGENT-PROMPT-PROTOTYPE-DEVELOPMENT.md`, `エージェントルール.md` | 完了（2026-03-29） |
...
| ID | タスク | 想定ルート / 備考 | ステータス |
|----|--------|-------------------|------------|
| DEV-L01 | **保有スキル一覧** — 一覧ページ（モックデータ） | `/learner/skill-hub/owned` | 完了（2026-03-28） |
| DEV-L02 | **スキルギャップ分析** — ギャップ一覧 → 詳細（モック） | `/learner/skill-hub/gap`, `/learner/skill-hub/gap/[code]` | 完了（2026-03-28） |
| DEV-L03 | **AI コースレコメンド** — ギャップ連動のレコメンド UI（モック） | `/learner/track/recommendations` | 完了（2026-03-28） |
| DEV-L04 | **学習パス自動生成** — レコメンドからパス生成結果画面（モック） | `/learner/track/learning-path` | 完了（2026-03-28） |
| DEV-L05 | **学習パス**（一覧・進捗ステップ） — Self-Learning 仕様の Locked/Active/Completed | `/learner/track/paths` | 完了（2026-03-28） |
| DEV-L06 | **学習履歴** | `/learner/track/history` | 完了（2026-03-28） |
| DEV-L07 | **通知 BOX** — ヘッダーまたは専用ページ | `Header` / `/learner/track/notifications` | 完了（2026-03-28） |
| DEV-L08 | コースカタログの **絞り込み** をギャップ / レコメンドと連動（クエリ or コンテキスト） | `app/learner/track/courses/page.tsx`（`?source=`） | 完了（2026-03-28） |
...
| DEV-L11 | スキル診断（**スキル調査・試験**）のプレースホルダーページ | `/learner/skill-hub/diagnostics/survey`, `/exam` | 完了（2026-03-28） |
| DEV-L12 | Skill Hub / Track 専用サイドバーと相互導線 | `SkillHubSidebar.tsx`, `TrackLearnerSidebar.tsx` | 完了（2026-03-28） |
...
| ID | タスク | 備考 | ステータス |
|----|--------|------|------------|
| DEV-I01 | 受講完了（モックアクション）で **スキル取得イベント** を発火 | `LearnerProgressContext`（サンプル開始ページからモック発火可） | 一部完了（2026-03・拡張可） |
| DEV-I02 | 同上で **バッジ取得** をホーム / 保有スキルに反映 | `LearnerProgressContext` + 各画面の表示連携 | 未着手 |
| DEV-I03 | `05-MOCK-DATA-SCHEMA.md` に受講者用フィールド（例: `pendingApproval`, `recommendedFromGap`）を追記 | `docs/spec/05-MOCK-DATA-SCHEMA.md` + `mockData` | 未着手 |
...
- [ ] Header で受講者ビューに切替 → `/learner/skill-hub/home` が表示される
- [ ] 受講者から `/learner/track/courses` でフィルタ・カード操作ができる
...
| ドキュメント | 役割 |
|--------------|------|
| [00-REBUILD-SPECIFICATION.md](./spec/00-REBUILD-SPECIFICATION.md) | 全体・ルート一覧（要同期） |
| [04-PAGES-AND-FEATURES.md](./spec/04-PAGES-AND-FEATURES.md) | ページ別仕様（要同期） |
| [05-MOCK-DATA-SCHEMA.md](./spec/05-MOCK-DATA-SCHEMA.md) | 型とモックデータ |
| [research/](./research/) | 経営・LXP 調査（仕様外） |
